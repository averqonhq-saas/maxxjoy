...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        const adminEmails = ['muneeswaranmd2004@gmail.com', 'yogaprathap@maxxjoytours.com', 'george@maxxjoytours.com', 'info@maxxjoytours.com'];
        const emailLower = firebaseUser.email?.toLowerCase() || '';
        const isAdminUser = adminEmails.includes(emailLower) || emailLower.includes('admin');
        const userData = {
    ...
    const unsub = onSnapshot(doc(db, 'settings', 'legal'), (snap) => {
      if (snap.exists()) {
        ...
        const hasLegacyText = 
          ...
          data.privacyPolicy?.includes('perfecttravel') ||
          data.privacyPolicy?.includes('555') ||
          data.termsConditions?.includes('perfecttravel') ||
          data.termsConditions?.includes('555') ||
          data.cookiePolicy?.includes('perfecttravel') ||
          data.cookiePolicy?.includes('555');

        if (hasLegacyText) {
          const cleaned = {
            companyName: 'Maxx Joy Tours and Travel Pvt Ltd',
            phone: '+91 98047 77879 / +91 74184 07088',
            whatsapp: '+91 98047 77879',
            email: 'Info@maxxjoytours.com',
          ...
          setLegalSettings(prev => ({ ...prev, ...cleaned }));
        } else {
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        user, authLoading, logoutUser, isAdmin: user?.isAdmin || ['muneeswaranmd2004@gmail.com', 'yogaprathap@maxxjoytours.com', 'george@maxxjoytours.com', 'info@maxxjoytours.com'].includes(user?.email?.toLowerCase()),
